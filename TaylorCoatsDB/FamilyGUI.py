...
def view_command():
    list1.delete(0,END)
    for row in TaylorCoats.view():
        list1.insert(END,row)

#Function to call the search function in TaylorCoats.py and display results  in listbox
def search_command():
    list1.delete(0,END)
    for row in TaylorCoats.search(name_text.get(),phone_text.get(), email_text.get(),street_text.get(),city_text.get(),state_text.get(),zip_text.get()):
        list1.insert(END,row)

#Function to call the add function in TaylorCoats.py to add entry into database
def add_command():
    TaylorCoats.insert(name_text.get(),phone_text.get(), email_text.get(),street_text.get(),city_text.get(),state_text.get(),zip_text.get())
    list1.delete(0,END)
...
def delete_command():
    TaylorCoats.delete(selected_tuple[0])
    view_command()

#Function to call the update function in TaylorCoats.py to update entry into database
def update_command():
    TaylorCoats.update(selected_tuple[0],name_text.get(),phone_text.get(), email_text.get(),street_text.get(),city_text.get(),state_text.get(),zip_text.get())
    view_command()
